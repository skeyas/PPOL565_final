...
class CensusMethods:
	...
	def retrieve_tract_level_census_data_by_state_and_year(self, fields, state, county, tract, year):
	...
	def retrieve_bg_level_census_data_by_state_and_year(self, fields, state, county, bg, year):
		return self.c.acs5.state_county_blockgroup(fields = fields,
										  state_fips = eval(f"states.{state}.fips"),
										  county_fips = county,
										  blockgroup = bg,
										  year = year)									  
	...
	def retrieve_tract_level_census_data_for_list_of_states_by_year(self, fields, states, county, tract, year):
		return {state: self.retrieve_tract_level_census_data_by_state_and_year(fields, state, county, tract, year) for state in states}
		
	def retrieve_bg_level_census_data_for_list_of_states_by_year(self, fields, states, county, bg, year):
		return {state: self.retrieve_bg_level_census_data_by_state_and_year(fields, state, county, bg, year) for state in states}
	...
	def recode_column_names_for_retrieved_data(self, df, relevant_values):
		...
		d = d0.to_dict()
		d.update({'state':'state','county':'county','tract':'tract', 'block_group': 'block_group'})
